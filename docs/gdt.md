...
**Source:** `src/arch/x86/cpu/gdt.rs`, `arch/x86/asm/gdt.asm`
...
### Anatomy of an entry (`src/arch/x86/cpu/gdt.rs:27-36`)
...
Our two real entries (`src/arch/x86/cpu/gdt.rs:69-82`):
...
So limit `0xFFFFF` pages × 4 KiB = **4 GiB**, base `0x00000000` — the flat model. Both entries are unit-tested against exactly these bytes (`src/arch/x86/cpu/gdt.rs:130-157`: `0x9A/0xCF` and `0x92/0xCF`).
...
These constants (`src/arch/x86/cpu/gdt.rs:11-12`) are reused by the IDT (`SELECTOR_KERNEL_CODE = 0x08`) — the tables agree on who's who.

### The load sequence (`src/arch/x86/cpu/gdt.rs:97-111` + `arch/x86/asm/gdt.asm`)
...
i686_GDT_Initialize (Rust)
  ...
  ▼
i686_GDT_Load (NASM, arch/x86/asm/gdt.asm:5-34)
  │  lgdt [eax]                    ; load GDTR
...
- `const _: () = assert!(size_of::<GdtEntry>() == 8)` (`src/arch/x86/cpu/gdt.rs:46-47`) — if anyone breaks the layout, **the build fails at compile time**, not in QEMU three layers deep.
- `GdtEntry::new` is a `const fn` (`src/arch/x86/cpu/gdt.rs:51-61`): descriptors are built at compile time into the `static GDT` table — zero runtime init cost, same job the C macro `GDT_ENTRY` did.

### Why `static mut` is acceptable here

`GDT` is `static mut` — a global the borrow checker can't guard. The justification lives in the SAFETY comment (`src/arch/x86/cpu/gdt.rs:63-64, 99-103`): the table must live for the entire kernel lifetime (the GDTR points into it forever), it's initialized exactly once during boot **under `cli`** from `_start`, and there is no concurrency yet. When SMP or preemption arrives, this invariant needs revisiting — the comment marks where.
